...
public class LightningAppBuilderPageBusinessLayer extends LightningAppBuilderPage {
	...
	public boolean createNewListView(String Title,String fieldTyppe, String operator,String value) {

		boolean flag = false;
		
		if(click(driver, getcreateNewView(20), "", action.BOOLEAN)) {
			log(LogStatus.PASS, "able to click on new list view button", YesNo.No);
			ThreadSleep(5000);
			switchToFrame(driver, 60, getLocator(120));
			ThreadSleep(5000);
			if(CommonLib.sendKeys(dDriver, getViewName(20), Title, "fieldName", action.BOOLEAN)) {
				ThreadSleep(2000);
				log(LogStatus.PASS, "able to enter view name ", YesNo.No);
				if (CommonLib.selectVisibleTextFromDropDown(driver,getFieldDropDown(20), "view field dropdown", fieldTyppe)) {
					log(LogStatus.INFO, "Selected the field dropdown", YesNo.No);
					ThreadSleep(2000);
					if (CommonLib.selectVisibleTextFromDropDown(driver,getViewOperatorDropDown(20), "view operator dropdown", operator)) {
						log(LogStatus.INFO, "Selected the view operator dropdown", YesNo.No);
						ThreadSleep(2000);
						if(CommonLib.sendKeys(dDriver, getViewValueBox(20), value, "value input", action.BOOLEAN)) {
							log(LogStatus.PASS, "able to enter view value ", YesNo.No);
							ThreadSleep(2000);
							if(click(driver, saveButton(20), "save", action.BOOLEAN)) {
								log(LogStatus.PASS, "able to click on save button", YesNo.No);
								flag=true;
							}else {
							...
							log(LogStatus.ERROR, "not able to enter view value  ", YesNo.Yes);
							return flag;
						}
					} else {
						log(LogStatus.ERROR, "Not Able To Select view operator dropdown", YesNo.No);
						return flag;

					}
				} else {
					log(LogStatus.ERROR, "Not Able To Select field dropdown", YesNo.No);
					return flag;

				}
			}else {
				log(LogStatus.ERROR, "not able to enter view name ", YesNo.Yes);
				return flag;
			}
		}else {
			log(LogStatus.ERROR, "not able to click on new list view button", YesNo.Yes);
			return flag;
		}
