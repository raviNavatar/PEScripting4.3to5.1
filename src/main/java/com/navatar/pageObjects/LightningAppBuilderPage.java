...
public class LightningAppBuilderPage extends BasePageBusinessLayer {
	...
	@FindBy(xpath="//div[@class='bFilterView']//a[contains(text(),'New View')]")
	private WebElement createNewView;

	public WebElement getcreateNewView(int timeOut) {
		return isDisplayed(driver, createNewView, "Visibility", timeOut, "createNewView");

	}
	
	@FindBy(xpath="//input[@id='fname']")
	private WebElement viewNameInput;

	public WebElement getviewNameInput(int timeOut) {
		return isDisplayed(driver, createNewView, "Visibility", timeOut, "viewNameInput");

	}
	
	@FindBy(xpath="//select[@id='fcol1']")
	private WebElement filterFieldName;

	public WebElement getfilterFieldName(int timeOut) {
		return isDisplayed(driver, filterFieldName, "Visibility", timeOut, "filterFieldName");

	}
	
	@FindBy(xpath="//select[@id='fop1']")
	private WebElement filterOperatorName;

	public WebElement getfilterOperatorName(int timeOut) {
		return isDisplayed(driver, filterOperatorName, "Visibility", timeOut, "filterOperatorName");
	...
	public WebElement getfiltervalueInput(int timeOut) {
		return isDisplayed(driver, filtervalueInput, "Visibility", timeOut, "filtervalueInput");
	...
	public List<WebElement> getAllHomepageList() {
		boolean flag = false;
		String xPath = "";
		WebElement ele = null;
		int count =0;
		ThreadSleep(5000);
		List<WebElement> allOtherAssignmentLabelValue =new ArrayList<WebElement>();
		
		xPath = "//td[@class='actionColumn']/following-sibling::th[@scope='row']/a";
		List<WebElement> allHeader= FindElements(driver, xPath);
		for(int i=0;i<allHeader.size();i++) {
		String headerName=	allHeader.get(i).getText().trim();
		if(headerName.contains("Delete")) {
			allHeader.remove(i);
			break;
		}
		}
		allOtherAssignmentLabelValue.addAll(allHeader);
